...
void KeyExpansion(const byte *cipher_key)
{
...
void InvMixColumns()
{
	...
	memcpy(temp, State, 16); 	//09 0e 0b 0d                                                      		//0d 09 0e 0b             		
...
int encrypt(const byte plain_text[TEXT_LEN], byte cipher_text[TEXT_LEN], const byte *cipher_key, int key_len)
{
	SetNbNkNr(key_len);		//设置密钥块数，轮数 
	KeyExpansion(cipher_key);	//密钥扩展，必须提前做的初始化

	memcpy(State, plain_text, TEXT_LEN);
	AddRoundKey(0);			//轮密钥加
	...
	// cipher_text = state
	memcpy(cipher_text, State, TEXT_LEN);
	return 0;
}

int decrypt(const byte cipher_text[TEXT_LEN], byte plain_text[TEXT_LEN], const byte *cipher_key, int key_len)
{
	SetNbNkNr(key_len);		//设置密钥块数，轮数 
	KeyExpansion(cipher_key);	//密钥扩展，必须提前做的初始化

	memcpy(State, cipher_text, TEXT_LEN);
	AddRoundKey(Nr);
	...
	// cipher_text = state
	memcpy(plain_text, State, TEXT_LEN);
	return 0;
}
